...
interface LoadStateProps {
  ...
  emptyTitle?: string;
  emptyMessage?: string;
  /** Optional action rendered inside the empty state (e.g. a "New project" CTA). */
  emptyAction?: ReactNode;
  /** Retry the failed fetch (react-query refetch). */
  onRetry?: () => void;
  /** Content-shaped loading placeholder. Defaults to card-row skeletons; pass a
   * bespoke skeleton (e.g. a board) for non-list screens. */
  skeleton?: ReactNode;
  children: ReactNode;
}
 ...
 * there's nothing, and otherwise the children. Error takes priority over empty.
 * Stops screens from hanging on a spinner or silently blanking when a fetch fails. */
